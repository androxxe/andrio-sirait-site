      ...
      <div className="w-full h-80 relative border-2 border-slate-200 rounded-lg hover:scale-[101%] ease-out duration-100 group/portfolio">
        <div className="absolute top-0 right-0 z-10 mt-2 mx-2 flex flex-wrap gap-1 h-10 justify-end">
          {portfolio.tech_stack.map((tech_stack, index) => (
            <div key={index} className="h-[22px] w-[30px] bg-white rounded shadow-sm border-[0.5px] border-slate-300 overflow-hidden relative">
              <Image
                quality={75}
        ...
        <div className="w-full h-52 relative overflow-hidden">
          <Image className="object-cover" src={portfolio.thumbnail} fill={true} quality={80} loading="lazy" alt={`Thumbnail ${portfolio.name}`} sizes="100vw" />
        </div>
